...
import { FormsModule } from '@angular/forms';
import { ActivatedRoute, Router } from '@angular/router';
import { catchError, concatMap, EMPTY, from, map, Observable, startWith, of } from 'rxjs';
import { TownAddAccessRightsComponent } from 'src/app/_core/shared/components/town-add-access-rights/town-add-access-rights.component';
import { TownVehicleComponent } from 'src/app/_core/shared/components/town-vehicle/town-vehicle.component';
import { MaterialModule } from 'src/app/_core/shared/material/material.module';
import { AddonDataSource, Vehicle, AdditionalAccessRightsData, ServiceProvider } from 'src/app/models/common.model';
...
export class ServiceProviderComponent implements OnInit, OnDestroy {
  ...
  // NEW METHOD: Handle flat assignment from child component
  handleFlatAssignment(event: any) {
  if (event.action === 'save') {
    this.saveFlatAssignment(event.data);
  }
}

  // NEW METHOD: Save flat assignment
  saveFlatAssignment(flatsArray: any[]) {
  // Create comma-separated string: "Falcon-A:101,Falcon-B:102"
  const flatReference = flatsArray
    .map(flat => `${flat.buildingName}:${flat.flatNumber}`)
    .join(',');

  this.authService.postServiceProviderReferenceFlats(
    this.serviceProvider.idNumber,
    flatReference
  ).subscribe({
    next: (res) => {
      this.alertService.openSuccess('Flats assigned successfully');
    },
    error: (err) => {
      console.error('Error assigning flats:', err);
      this.errorHandlerService.handleError(err);
    }
  });
}
