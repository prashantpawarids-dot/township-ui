...
@Component({
  selector: 'app-card-printing-log',
  standalone: true,
  imports: [
     CommonModule,
    ReactiveFormsModule,
  ...
  templateUrl: './card-printing-log.component.html',
  styleUrls: ['../../../../table.css']
})
export class CardPrintingLogComponent implements OnInit {
  ...
  private loadAllCardData(): void {
    this.http.get<any[]>(`${environment.apiurl}Report/CardPrintingLog`).subscribe({
      next: data => {
        this.allData = data;
        this.filteredData = [];
        this.displayedData = [];
  ...
  const dateRangeText = minDate && maxDate
    ? `Card Printing Date From: ${minDate.toLocaleDateString()} - To: ${maxDate.toLocaleDateString()}`
    : '';
  ...
  logo.onload = () => {
    ...
    doc.text('CARD PRINTING', pageWidth / 2, 20, { align: 'center' });
...
'Card Printing Date': d.cardPrintingDate ? DateTimeUtil.formatDateTime(d.cardPrintingDate) : '',
    ...
    const wsData: any[][] = [
      ["CARD PRINTING", "", "", "", "", "", "", "", "", `Printed On: ${printedDate}`],
      [],
