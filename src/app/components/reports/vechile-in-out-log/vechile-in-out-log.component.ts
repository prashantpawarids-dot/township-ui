...
//   showReports() {
//     const { fromDate, toDate, vehicleType, inOut, searchBy, searchValue, project, building } = this.reportForm.value;
//     let filtered = [...this.vehicleData];

//     // if (fromDate) filtered = filtered.filter(x => this.fixDateFormat(x.inoutTime).getTime() >= new Date(fromDate).getTime());
//     // if (toDate) filtered = filtered.filter(x => this.fixDateFormat(x.inoutTime).getTime() <= new Date(toDate).getTime());
//     if (fromDate) {
//   const from = new Date(fromDate);
//   filtered = filtered.filter(x => this.fixDateFormat(x.inoutTime).getTime() >= from.getTime());
// }

// if (toDate) {
//   const to = new Date(toDate);
//   to.setHours(23, 59, 59, 999); // set to end of day
//   filtered = filtered.filter(x => this.fixDateFormat(x.inoutTime).getTime() <= to.getTime());
// }
...
//     }
// // 🔹 SORT BY TIME ONLY (IMPORTANT)
//   filtered = sortByTimeOnly(filtered, 'inoutTime', 'asc');   // or 'desc'
