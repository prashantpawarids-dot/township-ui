...
filtered = filtered.filter(item => {
 ...
 exportPDF() {
  ...
  logo.onload = () => {
    ...
    // From-To below title (centered)
    const validFromDates = this.filteredData
      .map(r => r.validFromDate ? new Date(r.validFromDate).getTime() : null)
      .filter(d => d);
    const validToDates = this.filteredData
      .map(r => r.validToDate ? new Date(r.validToDate).getTime() : null)
      .filter(d => d);

    if (validFromDates.length || validToDates.length) {
      const minFrom = validFromDates.length ? new Date(Math.min(...validFromDates)).toLocaleDateString() : '';
      const maxTo = validToDates.length ? new Date(Math.max(...validToDates)).toLocaleDateString() : '';
      doc.setFontSize(11);
      doc.setFont(undefined, 'normal');
      doc.text(`From: ${minFrom} To: ${maxTo}`, pageWidth / 2, 28, { align: 'center' });
    }
    ...
    autoTable(doc, {
      startY: 35,
      head: [['Sr No', 'Short Name', 'Mobile', 'Service Type', 'First Name', 'Last Name', 'Joining Date', 'Valid From', 'Valid To', 'Email']],
      body,
      theme: 'grid',
