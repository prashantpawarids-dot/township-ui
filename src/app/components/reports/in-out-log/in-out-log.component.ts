...
export class InOutLogComponent implements OnInit, AfterViewInit {
  ...
  searchTypes = [
    ...
    { value: 'nrdName', label: 'Neighbourhood' },
...
//   // Only listen for changes but don't populate table automatically
//   this.reportForm.get('searchValue')?.valueChanges.subscribe(() => {
//     // optional: you can call applyAutoFilter if you want instant filtering after search value input
//   });
...
//     this.reportForm.get('buildingName')?.setValue(filteredSelection);
//   });
// }
ngOnInit() {
...
//   // --------------------------
//   if (f.accessMode) {
//     data = data.filter(x => x.accessmode?.toUpperCase() === f.accessMode.toUpperCase());
//   }
...
//     data = data.filter(x => f.cardType.includes(x.cardType));
//   }
...
//     // If ALL selected, only match NRDs that exist in the main API
