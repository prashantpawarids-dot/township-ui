...
//   // Listen for roleID changes and update name field
//   this.userForm.get('roleID')?.valueChanges.subscribe(selectedId => {
//     const match = this.profilenameOptions?.find(opt => opt.id == selectedId);
//     if (match) {
//       this.userForm.patchValue({ name: match.profileName }, { emitEvent: false });
//     }
//   });
...
ngOnInit() {
  ...
  // uid typed → send to name
  this.userForm.get('uid')?.valueChanges.subscribe(uidValue => {
    this.userForm.patchValue({ name: uidValue }, { emitEvent: false });
  });
  ...
  const match = this.profilenameOptions?.find(opt => opt.id == selectedId);
  if (match) {
    const profileKey = match.profileName?.toLowerCase()?.trim();
    const roleMap: { [key: string]: number } = {
      'admin':   1,
      'manager': 2,
      'user':    3,
    ...
    this.userForm.patchValue({
      role:        null,                        // ← send null, API handles Role object
      roleID:      roleMap[profileKey] ?? selectedId,  // ← 1/2/3 for API
  ...
  let match = this.profilenameOptions?.find(opt => opt.id == roleID);
  // fallback: match by roleMap number (1/2/3)
  if (!match) {
    const reverseMap: { [key: number]: string } = { 1: 'admin', 2: 'manager', 3: 'user' };
    const profileKey = reverseMap[roleID];
    match = this.profilenameOptions?.find(opt => opt.profileName?.toLowerCase() == profileKey);
  }
